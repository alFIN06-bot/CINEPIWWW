  ...
  const initLoginForm = () => {
    ...
    if (loginForm) {
      loginForm.addEventListener('submit', async (event) => {
        ...
        // Menjadi:
        if (email !== '' && password !== '') {
          // Ini akan selalu true jika email dan password tidak kosong
          alert('Login berhasil! Selamat datang.');
          window.location.href = 'profile-166.html';
        } else {
          alert('Login gagal: Email atau password salah.'); // Baris ini mungkin tidak akan pernah tercapai dengan kondisi di atas
        }
      });
    }
  };
    ...
    if (filmDetailContainer) {
      ...
      const filmsData = {
        'warkop-dki-reborn': {
          ...
          rating: '9.5', // Changed rating to 9.5
          genre: 'Komedi',
